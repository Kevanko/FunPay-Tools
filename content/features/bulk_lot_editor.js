...
async function openBulkEditor() {
    ...
    overlay.innerHTML = `
        <div class="fp-tools-modal-content fpt-modal-surface" style="max-width:720px;width:95%;max-height:90vh;display:flex;flex-direction:column;">
            <div class="fp-tools-modal-header" style="padding:16px 20px;border-bottom:1px solid var(--fpt-pLine,#1e2030);">
                <h3 style="margin:0;font-size:15px;color:var(--fpt-pTx,#e8eaf0);">Массовое редактирование лотов</h3>
                <button class="fp-tools-modal-close" style="background:none;border:none;color:var(--fpt-pTxDim,#9099b8);font-size:24px;cursor:pointer;line-height:1;">×</button>
            </div>
            <div class="fp-tools-modal-body" style="overflow-y:auto;padding:16px 20px;flex:1;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">
                    <div>
                        <label style="font-size:12px;color:var(--fpt-pTxFaint,#5a5f7a);display:block;margin-bottom:4px;">Новое название (пусто - не менять)</label>
                        <input type="text" id="fp-bulk-new-name" placeholder="{current}" style="width:100%;background:var(--fpt-pInput,#0e0f16);border:1px solid var(--fpt-pLine,#22253a);border-radius:6px;padding:8px;color:var(--fpt-pTx,#d8dae8);font-size:13px;">
                    </div>
                    <div>
                        <label style="font-size:12px;color:var(--fpt-pTxFaint,#5a5f7a);display:block;margin-bottom:4px;">Сообщение покупателю (пусто - не менять)</label>
                        <input type="text" id="fp-bulk-new-msg" placeholder="Не изменять" style="width:100%;background:var(--fpt-pInput,#0e0f16);border:1px solid var(--fpt-pLine,#22253a);border-radius:6px;padding:8px;color:var(--fpt-pTx,#d8dae8);font-size:13px;">
                    </div>
                </div>

                <div style="margin-bottom:12px;">
                    <label style="font-size:12px;color:var(--fpt-pTxFaint,#5a5f7a);display:block;margin-bottom:4px;">Новое описание (пусто - не менять)</label>
                    <textarea id="fp-bulk-new-desc" placeholder="Не изменять" rows="3" style="width:100%;background:var(--fpt-pInput,#0e0f16);border:1px solid var(--fpt-pLine,#22253a);border-radius:6px;padding:8px;color:var(--fpt-pTx,#d8dae8);font-size:13px;resize:vertical;"></textarea>
                </div>

                <div style="border:1px solid var(--fpt-pLine,#1e2030);border-radius:8px;padding:12px;margin-bottom:16px;">
                    <label style="font-size:12px;color:var(--fpt-pTxFaint,#5a5f7a);display:block;margin-bottom:8px;font-weight:700;">Найти и заменить (точечно, без перезаписи всего)</label>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px;">
                        <input type="text" id="fp-bulk-find" placeholder="Найти…" style="width:100%;background:var(--fpt-pInput,#0e0f16);border:1px solid var(--fpt-pLine,#22253a);border-radius:6px;padding:8px;color:var(--fpt-pTx,#d8dae8);font-size:13px;">
                        <input type="text" id="fp-bulk-replace" placeholder="Заменить на…" style="width:100%;background:var(--fpt-pInput,#0e0f16);border:1px solid var(--fpt-pLine,#22253a);border-radius:6px;padding:8px;color:var(--fpt-pTx,#d8dae8);font-size:13px;">
                    </div>
                    <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;font-size:12px;color:var(--fpt-pTxDim,#9099b8);">
                        <span style="color:var(--fpt-pTxFaint,#5a5f7a);">Применять к:</span>
                        <label style="display:flex;gap:5px;align-items:center;cursor:pointer;"><input type="checkbox" id="fp-bulk-fr-name" checked style="accent-color:var(--fpt-uacc,#C026D3);"> названию</label>
                        <label style="display:flex;gap:5px;align-items:center;cursor:pointer;"><input type="checkbox" id="fp-bulk-fr-desc" checked style="accent-color:var(--fpt-uacc,#C026D3);"> описанию</label>
                        <label style="display:flex;gap:5px;align-items:center;cursor:pointer;"><input type="checkbox" id="fp-bulk-fr-msg" style="accent-color:var(--fpt-uacc,#C026D3);"> сообщению</label>
                    </div>
                    <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;font-size:12px;color:var(--fpt-pTxDim,#9099b8);margin-top:8px;">
                        <label style="display:flex;gap:5px;align-items:center;cursor:pointer;"><input type="checkbox" id="fp-bulk-fr-regex" style="accent-color:var(--fpt-uacc,#C026D3);"> RegEx</label>
                        <label style="display:flex;gap:5px;align-items:center;cursor:pointer;"><input type="checkbox" id="fp-bulk-fr-case" style="accent-color:var(--fpt-uacc,#C026D3);"> учитывать регистр</label>
                        <label style="display:flex;gap:5px;align-items:center;cursor:pointer;"><input type="checkbox" id="fp-bulk-fr-word" style="accent-color:var(--fpt-uacc,#C026D3);"> только целые слова</label>
                        <label style="display:flex;gap:5px;align-items:center;cursor:pointer;"><input type="checkbox" id="fp-bulk-fr-all" checked style="accent-color:var(--fpt-uacc,#C026D3);"> все совпадения</label>
                    </div>
                    <div style="font-size:11px;color:var(--fpt-pTxFaint,#5a5f7a);margin-top:8px;">Работает вместе с полями выше: сначала «найти-заменить», затем подстановка полных значений (если заданы). При RegEx в «Заменить на» доступны <code>$1</code>, <code>$2</code> и т.д.</div>
                </div>

                <div style="border:1px solid var(--fpt-pLine,#1e2030);border-radius:8px;padding:12px;margin-bottom:16px;">
                    <label style="font-size:12px;color:var(--fpt-pTxFaint,#5a5f7a);display:block;margin-bottom:8px;font-weight:700;">Изменение цены</label>
                    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                        <select id="fp-bulk-price-mode" style="background:var(--fpt-pInput,#0e0f16);border:1px solid var(--fpt-pLine,#22253a);border-radius:6px;padding:8px;color:var(--fpt-pTx,#d8dae8);font-size:13px;">
                        ...
                        <input type="number" id="fp-bulk-price-value" step="0.01" placeholder="0" disabled style="flex:1;min-width:120px;background:var(--fpt-pInput,#0e0f16);border:1px solid var(--fpt-pLine,#22253a);border-radius:6px;padding:8px;color:var(--fpt-pTx,#d8dae8);font-size:13px;">
                        <span id="fp-bulk-price-unit" style="font-size:12px;color:var(--fpt-pTxFaint,#5a5f7a);min-width:14px;"></span>
                    </div>
                    <div style="display:flex;gap:14px;align-items:center;margin-top:8px;flex-wrap:wrap;">
                        <label style="font-size:12px;color:var(--fpt-pTxDim,#9099b8);display:flex;align-items:center;gap:5px;cursor:pointer;"><input type="checkbox" id="fp-bulk-price-round" style="accent-color:var(--fpt-uacc,#C026D3);"> Округлять до целого</label>
                        <label style="font-size:12px;color:var(--fpt-pTxDim,#9099b8);display:flex;align-items:center;gap:5px;cursor:pointer;"><input type="number" id="fp-bulk-price-min" step="0.01" placeholder="мин." style="width:80px;background:var(--fpt-pInput,#0e0f16);border:1px solid var(--fpt-pLine,#22253a);border-radius:6px;padding:5px;color:var(--fpt-pTx,#d8dae8);font-size:12px;"> не ниже</label>
                    </div>
                </div>

                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    <span style="font-size:12px;color:var(--fpt-pTxFaint,#5a5f7a);">Лоты (<span id="fp-bulk-count">${lots.length}</span> / выбрано <span id="fp-bulk-selected-count">0</span>):</span>
                    <div style="display:flex;gap:6px;align-items:center;">
                        <input type="text" id="fp-bulk-filter" placeholder="Фильтр…" style="background:var(--fpt-pInput,#0e0f16);border:1px solid var(--fpt-pLine,#22253a);border-radius:6px;padding:5px 8px;color:var(--fpt-pTx,#d8dae8);font-size:12px;width:130px;">
                        <button id="fp-bulk-select-all" class="btn btn-default" style="padding:4px 10px;font-size:12px;">Выбрать все</button>
                    </div>
                </div>

                <div id="fp-bulk-lots-list" style="border:1px solid var(--fpt-pLine,#1e2030);border-radius:8px;max-height:240px;overflow-y:auto;">
                    ${lots.map((lot) => `
                        <label class="fp-bulk-lot-row" data-title="${(lot.title || '').toLowerCase().replace(/"/g,'&quot;')}" style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-bottom:1px solid var(--fpt-pLine,#1e2030);cursor:pointer;font-size:13px;color:var(--fpt-pTxDim,#9099b8);">
                            <input type="checkbox" class="fp-bulk-lot-check" data-offer-id="${lot.id}" data-node-id="${lot.nodeId}" style="accent-color:var(--fpt-uacc,#C026D3);flex-shrink:0;">
                            <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${lot.title}</span>
                            <span style="font-size:11px;color:var(--fpt-pTxFaint,#4a4f68);flex-shrink:0;">${lot.categoryName}</span>
                        </label>
                    `).join('')}
                </div>

                <div id="fp-bulk-progress" style="display:none;margin-top:14px;">
                    <div style="height:4px;background:var(--fpt-p3,#1e2030);border-radius:2px;overflow:hidden;">
                        <div id="fp-bulk-progress-bar" style="height:100%;background:var(--fpt-uacc,#C026D3);width:0;transition:width 0.3s;border-radius:2px;"></div>
                    </div>
                    <div id="fp-bulk-progress-text" style="font-size:12px;color:var(--fpt-pTxFaint,#5a5f7a);margin-top:6px;text-align:center;"></div>
                    <div id="fp-bulk-log" style="font-size:11px;color:var(--fpt-pTxDim,#6b7194);margin-top:8px;max-height:90px;overflow-y:auto;font-family:monospace;line-height:1.5;"></div>
                </div>
            </div>
            <div class="fp-tools-modal-footer" style="padding:14px 20px;display:flex;gap:10px;border-top:1px solid var(--fpt-pLine,#1e2030);">
